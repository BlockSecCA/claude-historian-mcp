...
export class ConversationParser {
  ...
  private extractActionItems(content: string): string[] {
    ...
    const actionPatterns = [
      ...
      /\d+\.\s+([^\n]{15,150})/g, // Numbered lists — capture to end of line
      /[-*]\s+([^\n]{15,150})/g, // Bullet points — capture to end of line
    ];
    ...
    actionPatterns.forEach((pattern) => {
      ...
      while ((match = pattern.exec(content)) !== null) {
        if (match[1] && match[1].trim().length > 10) {
          ...
          // If truncated at 150 chars, back up to last word boundary
          if (action.length >= 149 && !action.endsWith(' ')) {
  ...
  // Preserve the most valuable content while maintaining original reading order
  private extractMostValuableContent(content: string, maxLength: number): string {
    ...
    // For conversational content: keep from the beginning, truncate at a natural boundary
    // This preserves the original reading order instead of reordering by "value score"
    if (content.length <= maxLength) {
      return content;
    }

    // Try to truncate at natural paragraph/sentence boundaries
    const boundaries = ['\n\n', '.\n', '. ', '!\n', '! ', '?\n', '? ', '\n'];

    for (const boundary of boundaries) {
      const lastBoundary = content.lastIndexOf(boundary, maxLength - 3);
      if (lastBoundary > maxLength * 0.6) {
        return content.substring(0, lastBoundary + boundary.length).trimEnd() + '...';
      }
    }

    // Fallback: truncate at last space before limit
